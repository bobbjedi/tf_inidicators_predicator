...
export const getUrl = (market: MarketName, pairName: string, count_candels: number, interval: number, endTime = 0) => {
  ...
  case ('binance'):
    ...
    url = 'https://api.binance.com/api/v1/klines?limit=' + count_candels + '&symbol=' + symbol + '&interval=' + Interval + (endTime ? '&endTime=' + endTime : '')
...
export async function getCandels (market: MarketName, pairName: string, count_candels: number, interval: number, end = 0) { // ('binance','BTC-USDT', 14, 15) баржа, пара, период, TF в минутах

  const URL = getUrl(market, pairName, count_candels, interval, end)
  console.log(URL)
