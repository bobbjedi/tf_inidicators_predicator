...
const prepInputFromCandels = (candels: Candel[]) => {
  const inp = candels.map((c, i) => {
    return candels[i + 1] ? $u.mathChangedLast2Candels([c, candels[i + 1]]).price : Infinity
  }).filter(v => v < Infinity)
...
export const getUrl = (market: MarketName, pairName: string, count_candels: number, interval: number) => {
  ...
  let convert = false
  const start_ = +(new Date().getTime() / 1000).toFixed(0) - count_candels * interval - 20 * 3600
  switch (market) {
  case ('binance'):
  ...
  case ('poloniex'):
    ...
    url = 'https://poloniex.com/public?command=returnChartData&currencyPair=' + symbol + '&start=' + start_ + '&end=9999999999&period=' + Interval
