...
import { prepBB, prepSMA, prepWEMA } from './prepInput'
...
export const trainNet = async ({ symbol, tf_, countCandels, testCount, callback }: { symbol: string; tf_: number, countCandels: number, testCount: number, callback?: (a: Log) => void }) => {
  ...
  // const criticSet = data.map(s => {
  //   return {
  //     output: [$u.normalise(+$u.percentChange(s.price, s.bestPrice), normilizeDemension, -normilizeDemension)],
  //     input: [_.last(s.input), s.output]
  //   }
  // })
  // const critic = lstm(criticSet[0].input, criticSet[0].output)
  // await critic.fit(tf.tensor3d(criticSet.map(d => d.input)), tf.tensor2d(criticSet.map(d => d.output)), {
  //   epochs: 20,
  //   // batchSize: 32,
  //   callbacks: {
  //     onEpochEnd (epoch, log) {
  //       console.log('Critic', epoch, log)
  ...
  console.log('Test:', lstmNet.evaluate(tf.tensor3d(testData.map(d => d.input)), tf.tensor2d(testData.map(d => d.output))).toString())
...
const prep3d = (candels: Candel[], opt: { normilizeDemension: number, period: number }) => {
  ...
  const data = arrCandels.map(c_ => {
    const c = c_.slice()
    const input: number[][] = prepBB(c, period).splice(-period)
    // input.push(prepWEMA(c, period).splice(-period))
    // input.push(prepBB(c, period).splice(-period))
    const output = input.splice(-2)[0]
...
const checkIsUp = (pred: number[], curent: number[]) => {
  // return arr.reduce((res, c, i) => {
  //   return res && c < (arr[i + 1] || Infinity)
  // }, true)
  return pred[0] < curent[0] && pred[1] < curent[1] && pred[2] < curent[2]
}
