...
async function onClickTrainModel (symbol: string, tf: number, countCandels: number, testCount: number) {
  ...
  Plotly.plot(graph_plot, [{ x: times.map($u.formatDate), y: buy.map((r, i) => r ? prices[i] : pricesMin), name: 'Buy' }], { margin: { t: 0 } })
  Plotly.plot(graph_plot, [{ x: times.map($u.formatDate), y: sell.map((r, i) => r ? prices[i] : pricesMin), name: 'Sell' }], { margin: { t: 0 } })
